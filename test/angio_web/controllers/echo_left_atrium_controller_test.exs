defmodule AngioWeb.Echo_left_atriumControllerTest do
  ...
  describe "create echo_left_atrium" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.echo_left_atrium_path(conn, :create), echo_left_atrium: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.echo_left_atrium_path(conn, :create), echo_left_atrium: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Echo left atrium"
    end
  end
  ...
  describe "edit echo_left_atrium" do
    ...
    test "renders form for editing chosen echo_left_atrium", %{conn: conn, echo_left_atrium: echo_left_atrium} do
  ...
  describe "update echo_left_atrium" do
    ...
    test "redirects when data is valid", %{conn: conn, echo_left_atrium: echo_left_atrium} do
      conn = put(conn, Routes.echo_left_atrium_path(conn, :update, echo_left_atrium), echo_left_atrium: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, echo_left_atrium: echo_left_atrium} do
      conn = put(conn, Routes.echo_left_atrium_path(conn, :update, echo_left_atrium), echo_left_atrium: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Echo left atrium"
    end
  end
