defmodule AngioWeb.Chd_care_episodeControllerTest do
  ...
  describe "create chd_care_episode" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.chd_care_episode_path(conn, :create), chd_care_episode: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.chd_care_episode_path(conn, :create), chd_care_episode: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Chd care episode"
    end
  end
  ...
  describe "edit chd_care_episode" do
    ...
    test "renders form for editing chosen chd_care_episode", %{conn: conn, chd_care_episode: chd_care_episode} do
  ...
  describe "update chd_care_episode" do
    ...
    test "redirects when data is valid", %{conn: conn, chd_care_episode: chd_care_episode} do
      conn = put(conn, Routes.chd_care_episode_path(conn, :update, chd_care_episode), chd_care_episode: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, chd_care_episode: chd_care_episode} do
      conn = put(conn, Routes.chd_care_episode_path(conn, :update, chd_care_episode), chd_care_episode: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Chd care episode"
    end
  end
