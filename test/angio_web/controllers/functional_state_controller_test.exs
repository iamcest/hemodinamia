defmodule AngioWeb.Functional_stateControllerTest do
  ...
  @create_attrs %{funs_basic_adl: "some funs_basic_adl", funs_cognition_level: "some funs_cognition_level", funs_frailty_scale_csha: "some funs_frailty_scale_csha", funs_katz_index: "some funs_katz_index", funs_kccq12_score: "some funs_kccq12_score", funs_notes: "some funs_notes", funs_status_assess_dt: ~D[2010-04-17], funs_stroke_rankin_dt: ~D[2010-04-17], funs_stroke_rankin_modified: "some funs_stroke_rankin_modified", funs_walking_level: "some funs_walking_level"}
  ...
  describe "create functional_state" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.functional_state_path(conn, :create), functional_state: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.functional_state_path(conn, :create), functional_state: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Functional state"
    end
  end
  ...
  describe "edit functional_state" do
    ...
    test "renders form for editing chosen functional_state", %{conn: conn, functional_state: functional_state} do
  ...
  describe "update functional_state" do
    ...
    test "redirects when data is valid", %{conn: conn, functional_state: functional_state} do
      conn = put(conn, Routes.functional_state_path(conn, :update, functional_state), functional_state: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, functional_state: functional_state} do
      conn = put(conn, Routes.functional_state_path(conn, :update, functional_state), functional_state: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Functional state"
    end
  end
